  ...
  this.on('pipe', function (source) {
    ...
    if (receiver.destroyed) {
      ...
      if (source.destroy) {
        ...
        receiver.logger.info('no destroy() method for source')
      }
      return
    }
...
ReceiverStream.prototype._writev = function (chunks, cb) {
  this.receiver.logger.debug({ chunks }, 'delivering messages')
  steed.each(this, chunks, processMsg, cb)
}

ReceiverStream.prototype._write = function (chunk, encoding, cb) {
  this.receiver.logger.debug(chunk, 'delivering message')
