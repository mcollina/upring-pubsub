...
Receiver.prototype.sendPeer = function (peer, count, cb) {
  ...
  this.log.debug({ peer, count }, 'direct subscribing')
...
function ReceiverStream (receiver, peer, count) {
  ...
  this.on('pipe', function (source) {
    receiver.log.info('stream piped')
    this.source = source
    ...
    const resubscribe = () => {
      receiver.log.info('source stream closed, resubscribe')
...
ReceiverStream.prototype._writev = function (chunks, cb) {
  this.receiver.log.debug({ chunks }, 'delivering messages')
  steed.each(this, chunks, processMsg, cb)
}

ReceiverStream.prototype._write = function (chunk, encoding, cb) {
  this.receiver.log.debug(chunk, 'delivering message')
